 ...
 * by zod (see `UsdcAmountSchema`) — we trust that shape here.
 *
 * Lives in `@ade/shared` (zero runtime deps) so agent-side tools can compare
 * decimal strings without pulling in the server package.
 */
